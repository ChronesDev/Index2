...
namespace Index
{
	struct Color
	{
		struct RawColor
		{
			float RawR = 0, RawG = 0, RawB = 0, RawA = 1;
		};
		...
		Vec4F AsRawVec() const
		{ return Vec4F { R, G, B, A }; }

		static Color RGB(Vec3F rgb)
		{ return Color(rgb.X, rgb.Y, rgb.Z); }

		static Color RGB(Vec3I rgb)
		{ return Color(rgb.X, rgb.Y, rgb.Z); }
		...
		{
			return Color((rgb & 0xFF0000) >> 16, (rgb & 0xFF00) >> 8, (rgb & 0xFF));
		}

		static Color RGBA(Vec4F rgba)
		...
		static Color ARGB(Vec4F argb)
		{ return Color(argb.Y, argb.X, argb.W, argb.X); }

		static Color ARGB(int r, int g, int b, int a = 255)
		{ return Color(r, g, b, a); }
		...
		static Color Raw(Vec3F raw)
		{ return Color(RawColor { .RawR = raw.X, .RawG = raw.Y, .RawB = raw.Z }); }
