...
namespace Index::UI
{
	// DynamicSizeCache
	template<typename T>
	struct DynamicVec1Cache : Vec1<T>
	{
		using Vec1<T>::Vec1;
	};
	...
	// DynamicSizeCache
	template<typename T>
	struct DynamicVec2Cache : Vec2<T>
	{
		using Vec2<T>::Vec2;
	};
	...
	// DynamicSizeCache
	template<typename T>
	struct DynamicVec3Cache : Vec3<T>
	{
		using Vec3<T>::Vec3;
	};
	...
	// DynamicSizeCache
	template<typename T>
	struct DynamicVec4Cache : Vec4<T>
	{
		using Vec4<T>::Vec4;
	};
	...
	struct DynamicVec1
	{
		...
		bool operator==(DynamicVec1<T> &rhs)
		{ return GetCache() == rhs.GetCache(); }
	...
	struct DynamicVec2
	{
		...
		bool operator==(DynamicVec2<T> &rhs)
		{ return GetCache() == rhs.GetCache(); }
	...
	struct DynamicVec3
	{
		...
		bool operator==(DynamicVec3<T> &rhs)
		{ return GetCache() == rhs.GetCache(); }
	...
	struct DynamicVec4
	{
		...
		bool operator==(DynamicVec4<T> &rhs)
		{ return GetCache() == rhs.GetCache(); }
...
DynSize(t2 width, t1 height) : _Width((t2Cast)width), _Height((t1Cast)height) { }
	...
	public:
		...
		__forceinline Size GetCache(UIDynamic *that, UIContext *u)
		{
			...
			return {
				GetWidthCache(that, u), GetHeightCache(that, u)
			};
		}
	...
	public: INDEX_UI_DynSize_Constructor_All_Cast_FirstType_SecondType(float, int, float, float)
...
namespace Index::UI
{
	...
	struct UIContext
	{
		...
		template<class TType = UIElement>
		__forceinline WPtr <TType> Find(String path);

		template<class TType = UIElement>
		__forceinline WPtr <TType> TryFind(String path);
...
{
	return {
		Index::Min(v1.X, v2.X), Index::Min(v1.Y, v2.Y)
	};
}
...
{
	return {
		Index::Max(v1.X, v2.X), Index::Max(v1.Y, v2.Y)
	};
}
...
{
	return {
		Index::Min(v1.Width, v2.Width), Index::Min(v1.Height, v2.Height)
	};
}
...
{
	return {
		Index::Max(v1.Width, v2.Width), Index::Max(v1.Height, v2.Height)
	};
}
...
{
	return {
		v1.X != NullF ? v1.X : $else.X, v1.Y != NullF ? v1.Y : $else.Y
	};
}
...
{
	return {
		v1.Width != NullF ? v1.Width : $else.Width, v1.Height != NullF ? v1.Height : $else.Height
	};
}
...
Index::Size Index::UI::GetMaxSize(UIElement *e)
{
	...
	auto maxSize = e->MaxSize;
	return {
		size.Width != NullF ? size.Width : maxSize.Width, size.Height != NullF ? size.Height : maxSize.Height
	};
}
