...
namespace Index
{
    template <class... TTypes> struct Variant : public std::variant<TTypes...>
    {
    public:
    ...
    public:
        template <typename T> constexpr T& Get() { return std::get<T>(*this); }

        template <typename T> constexpr T& GetOr(T&& other) const
        {
            if (Has<T>())
                return Get<T>();
            else
        ...
        template <typename T> constexpr bool Has() const { return std::holds_alternative<T>(*this); }

        INDEX_Property(get = index) size_t Index;
