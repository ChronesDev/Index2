...
namespace Index::UI
{
	...
	struct FixedGridElement
	{
		...
		static Index::IPtr<T> New(Args &&args)
		{ return Index::INew<FixedGridElement>(std::forward<Args>(args)); } INDEX_UI_DefaultConstructor(
			FixedGridElement)
	...
	struct FixedGrid : virtual UIElement
	{
		...
		void Render(UIContext *u, Layout i) override
		{
			...
			for (auto &c : Content)
			{
				...
				Index::Size size {
					(float) columnSpan * columnSize, (float) rowSpan * rowSize,
				};
				c->Render(
					u, {
						.Area {
							r.X + (columnSize * (float) column), r.Y + (rowSize * (float) row), size
						}}
				);
			}
		}
	...
	struct Dock : virtual UIElement
	{
		...
		{
			return INew<DockHolder>(
				DockHolder::Args {
					DockSide::Left, std::forward<INDEX_UI_List>(content)
				}
			);
		}
		...
		static IPtr <DockHolder> Top(INDEX_UI_List content)
		{
			return INew<DockHolder>(
				DockHolder::Args {
					DockSide::Top, std::forward<INDEX_UI_List>(content)
				}
			);
		}
		...
		static IPtr <DockHolder> Right(INDEX_UI_List content)
		{
			return INew<DockHolder>(
				DockHolder::Args {
					DockSide::Right, std::forward<INDEX_UI_List>(content)
				}
			);
		}
		...
		static IPtr <DockHolder> Bottom(INDEX_UI_List content)
		{
			return INew<DockHolder>(
				DockHolder::Args {
					DockSide::Bottom, std::forward<INDEX_UI_List>(content)
				}
			);
		}
		...
		static IPtr <DockHolder> Fill(INDEX_UI_List content)
		{
			return INew<DockHolder>(
				DockHolder::Args {
					DockSide::Fill, std::forward<INDEX_UI_List>(content)
				}
			);
		}
	...
	struct Column : virtual UIElementHolder
	{
		...
		__forceinline ExpandedContentInfoResult GetExpandedContentInfoH(Layout i)
		{
			...
			for (auto &c : Content)
			{
				...
				if (flex > 0)
					ret.UnitWidth = Max(
						ret.UnitWidth, ptr->MeasureIntentSize(std::forward<Layout>(i)).Width / (float) flex
					);
			}
			return ret;
		}
		...
		MeasureIntentSizeCustomResult MeasureIntentSizeCustom(Layout i)
		{
			...
			return {
				.OtherSize{
					Index::Max(minSize.Width, width), minSize.Height
				}, .OtherCount = otherCount, .ExpandedInfo = expandedInfo, .AllExpandedWidth = allExpandedWidth
			};
		}
	};
	...
	struct Row : virtual UIElementHolder
	{
		...
		__forceinline ExpandedContentInfoResult GetExpandedContentInfoV(Layout i)
		{
			...
			for (auto &c : Content)
			{
				...
				if (flex > 0)
					ret.UnitHeight = Max(
						ret.UnitHeight, ptr->MeasureIntentSize(std::forward<Layout>(i)).Height / (float) flex
					);
			}
			return ret;
		}
		...
		MeasureIntentSizeCustomResult MeasureIntentSizeCustom(Layout i)
		{
			...
			return {
				.OtherSize {
					minSize.Width, Index::Max(minSize.Height, height)
				}, .OtherCount = otherCount, .ExpandedInfo = expandedInfo, .AllExpandedHeight = allExpandedHeight
			};
		}
	};
	...
	{
		using BuildFunc = Func<
		IPtr<UIElement>(UIContext
		* u,
		Layout i
		)>;
